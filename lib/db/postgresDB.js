...
class DB {
    ...
    async addTable(storeName) {
        if (!this._tables[storeName]) {
            let storeDesc;
            const logMatch = storeName.match(/(.*)_log$/);
            ...
            const store = () => {
                ...
                let storeDBName = storeName;
                switch (storeName) {
                    case "_sequences":
                    case "_versions":
                        storeDBName = "blank" + storeName;
                        delete schema.__v;
                }
...
class Table {
    ...
    async get(query, $user, options = {}) {
        ...
        const res = await this.model.findOne({ where, transaction, logging, attributes });
