...
function _emitEvents(eventName, arg1, arg2, arg3, subscribers) {
    if (Array.isArray(subscribers)) {
        for (let subscriber of subscribers) {
            ...
            let item = _copyItemForUser(readableProps, eventData);
...
function _copyItemForUser(readableProps, source) {
    let res = {};
        ...
        if (readableProps[prop] === true) {
            ...
            if (Array.isArray(subValue)) {
                ...
                for (let subItem of subValue) {
                    res[prop].push(_copyItemForUser(prop, subItem));
                }
                continue;
            }
            res[prop] = (_copyItemForUser(prop, subValue));
        }
    }
    return res;
}
