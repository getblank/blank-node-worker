...
function createHandler(store, item) {
    ...
    emitProxyEvents(createHandler, store, item);
}
...
function updateHandler(store, item, prevItem) {
    ...
    emitProxyEvents(updateHandler, store, item, prevItem);
}
...
function emitRefEvents(storeName, item, prevItem) {
    ...
    for (let p of refPairs) {
        ...
        if (item[p.ref.prop] === prevItem[p.ref.prop] && oppositeRef.populateIn) {
            ...
            updateHandler(p.oppositeStoreName, updateData);
        }
    }
}
