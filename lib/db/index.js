...
class DB extends EventEmitter {
    ...
    async _delete(storeDesc, item, user, options) {
        ...
        switch (dataSource.type) {
            ...
            case "postgres":
                return postgres.table(storeDesc.name).then(res => res.delete(item, user, options));
            case "file":
    ...
    delete(storeName, _id, options = {}, cbk) {
        ...
        p.then(async _user => {
            ...
            .then(() => {
                cbk();
                this.emit("delete", storeDesc.name, item);
    ...
    async find(storeName, request, options = {}) {
        ...
        try {
            ...
            try {
                switch (dataSource.type) {
                    case "file":
                        res = await configStore.getDataSource(storeDesc, this).find(request, user, options);
                        break;
                    case "postgres":
                        res = await postgres.table(storeDesc.name).then(res => res.find(request, user, options));
                        break;
                    case "mongo":
        ...
        switch (dataSource.type) {
            case "mongo": {
                if (typeof query === "string") {
                    return this.mongo.get(storeDesc.name, query, options);
                }
                ...
                const q = await filter.prepare(storeDesc, query, user);
                return this.mongo.get(storeDesc.name, q, options);
            }
            case "postgres":
                return postgres.table(storeDesc.name).then(res => res.get(query, user, options));
            case "file":
    ...
    async nextSequence(sequenceId) {
        ...
        return table.nextSequence(sequenceId);
    }

    async nextSequenceString(sequenceId, stringLength) {
        if (!stringLength) {
            stringLength = 6;
        }

        let res = await this.nextSequence(sequenceId);
    ...
    async populateAll(storeName, item, user, cb) {
        ...
        return this._populateAll(storeDesc, item, user, cb);
    }
        ...
        this.getUser(options.user || options.userId || "system")
            ...
            .then(async fullItem => {
                ...
                if (!options.noEmitUpdate) {
                    this.emit("update", storeName, cloneDeep(fullItem), prevItem && cloneDeep(prevItem));
                }
                const readableProps = await configStore.getReadablePropsForUser(storeDesc, user);
    ...
    _populateAll(storeDesc, _item, $user, cb) {
        const d =
            typeof cb !== "function"
                ? new Promise((f, r) => (cb = (e, d) => setImmediate(() => (e != null ? r(e) : f(d)))))
                : null;
            ...
            const p = () => {
                return new Promise(resolve => {
                    ...
                    for (let i = 0; i < keys.length; i++) {
                        const key = keys[i];
                        const prop = storeDesc.props[key];
                        let refList = false;
                        switch (prop.type) {
                            case "refList":
                                refList = true;
                        ...
                        for (const refKey of refKeys) {
                            const p = () => {
                                return new Promise(async resolve => {
                                    ...
                                    if (prop.store === "users") {
                                        return this.getUser(refKey, (err, data) => {
                                            if (err) {
                                                console.error(
                                                    `When populating in store ${storeDesc.name} itemId: ${
                                                        item._id
                                    ...
                                    if (!storesCache[prop.store]) {
                                        storesCache[prop.store] = await configStore.getStoreDesc(prop.store, $user);
                                    }

                                    const refStoreDesc = storesCache[prop.store];
                                    this._get(refStoreDesc, refKey)
                                        .then(data => {
                                            data = prop.populateIn.fn ? prop.populateIn.fn(data) : data;
                                            if (refList) {
                                                item[prop.populateIn.prop] = item[prop.populateIn.prop] || [];
