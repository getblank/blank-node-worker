...
class DB extends EventEmitter {
    ...
    find(storeName, request, options = {}, _cb) {
        ...
        const d =
            typeof _cb !== "function"
                ? new Promise((f, r) => (_cb = (e, d) => setImmediate(() => (e != null ? r(e) : f(d)))))
                : null;
        ...
        p.then(async user => {
            ...
            const readableProps = configStore.getReadablePropsForUser(storeDesc, user);
            ...
            if (dataSource) {
                p = configStore.getDataSource(storeDesc, this).find(request);
            } else {
                ...
                p = this.mongo.find(storeName, request);
            }
            ...
            .then(savedItem => {
                ...
                setImmediate(() => {
                    if (!insert && storeDesc.logging) {
                        this._logItem(storeDesc, savedItem, prevItem, user);
                    }
                });
    ...
    _getArchivedItem(storeDesc, query) {
        const archiveQuery = {
            query: {
                itemId: query._id,
        ...
        let item;
        return this.mongo
            .get(storeName, query._id)
            .then(res => {
                item = res;
                return this.mongo.get(`${storeName}_log`, { itemId: query._id, prevVer: query.__v });
            })
            .then(res => {
                return this.mongo.rawFindAll(`${storeName}_log`, archiveQuery);
            })
            .then(res => {
                for (let diff of res) {
    ...
    _logItem(storeDesc, item, prevItem) {
        ...
        return this.mongo.insert(`${storeDesc.name}_log`, logRecord, true);
    }
