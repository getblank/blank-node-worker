...
class Db extends EventEmitter {
    ...
    set(storeName, item, options = {}, cbk) {
        ...
        this.getUser(options.user || options.userId || "system")
            ...
            .then(_unlock => {
                ...
                unlock = _unlock;
                return db.get(storeName, item._id).catch(() => null); // because it can be no saved item
            })
            .then(_prevItem => {
    ...
    _logItem(storeDesc, item, prevItem) {
        ...
        return db.insert(`${storeDesc.name}_log`, logRecord, true);
    }
