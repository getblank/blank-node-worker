...
let db = require("./rawDb");
let configStore = require("../configStore");
let sync = require("../sync");
let uuid = require("node-uuid");
let EventEmitter = require("events");
let auth = require("../auth");
let validation = require("validation").default;
let iso8601 = /^\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d(\.\d{1,3})?(Z|[\+-][012]\d\:[012]\d)$/;
...
class Db extends EventEmitter {
    ...
    find(storeName, request, options = {}, _cb) {
        ...
        p.then(user => {
            ...
            console.debug(`[$db][find] query calculated: ${JSON.stringify(request.query)}. ${baseDebugMessage}`);
        ...
        this.getUser(options.user || options.userId || "system", (err, user) => {
            ...
            db.get(storeName, _id, (err, item) => {
                ...
                if (err && err.message === "Not found" && options.deleted) {
                    ...
                    return db.get(`${storeName}_deleted`, _id, (err, item) => {
    ...
    set(storeName, item, options = {}, cb) {
        ...
        this.getUser(options.user || options.userId || "system").then((_user) => {
            ...
            return new Promise((resolve) => {
                db.get(storeName, item._id, (err, res) => {
                    resolve(res);
                });
            });
        }).then((_prevItem) => {
            ...
            if (!options.noValidate) {
                ...
                if (Object.keys(validationResult || {}).length > 0) {
                    console.debug("[VALIDATION ERRORS]", storeName, "/", newItem._id, JSON.stringify(validationResult, null, "  "));
            ...
            return new Promise((resolve, reject) => {
                this._populateAndLoadVirtual(storeName, savedItem, storeDesc, user, options, (err, item) => {
