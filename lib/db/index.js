...
class Db extends EventEmitter {
    ...
    set(item, storeName, options = {}, cb) {
        ...
        this.getUser(options.user || options.userId || "system").then((_user) => {
            ...
            this._prepareItem(storeDesc.props, newItem, prevItem);
            if (err) {
                throw err;
            }
    ...
    _prepareItem(props, item, prevItem) {
        for (let p of Object.keys(item)) {
            if (p === "__v" || p === "salt" || p === "hashedPassword") {
                continue;
            }
            ...
            switch (propDesc.type) {
                ...
                case "string":
                    ...
                    break;
                case "object":
                    item[p] = this._prepareItem(propDesc.props, item[p]);
                    break;
                case "objectList":
                    for (let i = 0; i < (item[p] || []).length; i++) {
                        item[p][i] = this._prepareItem(propDesc.props, item[p][i]);
                    }
                    break;
                case "date":
                    // May be convert date to string or vice versa
                    break;
            }
        }
    }
