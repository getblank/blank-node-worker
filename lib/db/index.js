...
class DB extends EventEmitter {
    ...
    get(storeName, _id, options = {}, cb) {
        ...
        this.getUser(userOption, async (err, user) => {
            ...
            const { dataSource } = storeDesc;
            if (dataSource) {
                p = configStore.getDataSource(storeDesc, this).get(query, user, options);
            } else {
                if (Object.keys(query).length === 2 && query._id && query.__v) {
                    ...
                    p = this.mongo.get(storeName, q, options);
                }
            }
    ...
    set(storeName, item, options = {}, cbk) {
        ...
        this.getUser(options.user || options.userId || "system")
            ...
            .then(_unlock => {
                ...
                unlock = _unlock;
                if (storeDesc.dataSource) {
                    return configStore
                        .getDataSource(storeDesc, this)
                        .get({ _id: item._id }, user, options)
                        .catch(() => null);
                }
