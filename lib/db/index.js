...
class Db extends EventEmitter {
    ...
    _copyReadableItemProps(readableProps, source, patch = false) {
        if (patch) {
            const res = [];
            for (let item of source) {
                const { path: propPath } = item;
                const path = propPath.split("/");
                switch (path.length) {
                    case 2: {
                        if (readableProps[path[1]] === true) {
                            res.push(item);
                        }
                        break;
                    }
                    case 3: {
                        const parentProp = path[1];
                        const childProp = path[2];
                        if ((readableProps[parentProp] || {})[childProp] === true) {
                            res.push(item);
                        }
                        break;
                    }
