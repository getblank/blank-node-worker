...
class DB extends EventEmitter {
    ...
    rawFindOneAndUpdate(storeName, query, update, upsert, cb) {
        ...
        this.db.collection(storeName, { strict: false }, (err, collection) => {
            collection.findOneAndUpdate(query, update, { returnOriginal: false, upsert }, (err, res) => {
                if (err) {
                    return cb(err, null);
                }
                ...
                return cb(
                    new Error((res.lastErrorObject && (res.lastErrorObject.MongoError || res.lastErrorObject)) || res),
                    null
                );
            });
        });
    ...
    rawFindAll(store, query, cb) {
        ...
        this.db.collection(store, { strict: true }, (err, collection) => {
            ...
            if (query.props) {
                ...
                cursor = collection.find(q, { projection });
            } else {
