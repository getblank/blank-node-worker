...
var _blankConst = require("blankConst");
...
let valueComparableTypes = [_blankConst.propertyTypes.int, _blankConst.propertyTypes.float, _blankConst.propertyTypes.date],
    lengthComparableTypes = [_blankConst.propertyTypes.string, _blankConst.propertyTypes.password, _blankConst.propertyTypes.objectList],
    patternApplicableTypes = [_blankConst.propertyTypes.int, _blankConst.propertyTypes.float, _blankConst.propertyTypes.string, _blankConst.propertyTypes.password],
    maskApplicableTypes = [_blankConst.propertyTypes.string],
    nonValidatedTypes = [_blankConst.propertyTypes.virtual, _blankConst.propertyTypes.virtualRefList, _blankConst.propertyTypes.comments, _blankConst.propertyTypes.action];
...
function validateType(type, value, propName) {
    ...
    switch (type) {
        ...
        case _blankConst.propertyTypes.bool:
            typeError = typeof value !== "boolean";
            break;
        case _blankConst.propertyTypes.string:
        case _blankConst.propertyTypes.password:
            typeError = typeof value !== "string";
            break;
        case _blankConst.propertyTypes.date:
            typeError = (typeof value !== "string" || !value.match(_blankConst.iso8601)) && !(value instanceof Date);
            break;
        case _blankConst.propertyTypes.ref:
            typeError = typeof value !== "string";
            break;
        case _blankConst.propertyTypes.refList:
            ...
            break;
        case _blankConst.propertyTypes.object:
            typeError = typeof value !== "object";
            break;
        case _blankConst.propertyTypes.objectList:
            typeError = !Array.isArray(value);
            break;
        case _blankConst.propertyTypes.file:
        case _blankConst.propertyTypes.fileList:
            for (let upload of value || []) {
                if (upload.$uploadState === _blankConst.uploadStates.uploading) {
...
function validateProperty(propsDesc, item, propName, baseItem, user) {
    ...
    //Check validators
    for (let validatorName of Object.keys(_blankConst.baseValidators)) {
    ...
    if (typeError) {
        ...
        errors.push(getValidationError(_blankConst.validityErrors.TYPE_ERROR, message, null, value));
    }

    //Inner object
    if (propDesc.type === _blankConst.propertyTypes.object && value != null) {
        let err = validation.validate(propDesc.props, value, item, user);
        if (err && Object.keys(err).length > 0) {
            errors.push(getValidationError(_blankConst.validityErrors.INNER_ERROR, null, err));
        }
    }
    if (propDesc.type === _blankConst.propertyTypes.objectList && Array.isArray(value)) {
        let listErrors = [],
            push = false;
        for (let i = 0; i < value.length; i++) {
        ...
        if (push) {
            errors.push(getValidationError(_blankConst.validityErrors.INNER_ERROR, null, listErrors));
        }
    }
    ...
    if (propDesc.required != null) {
        ...
        if (validator.getValue(user, item, baseItem) && (value == null || value === "" || propDesc.type === _blankConst.propertyTypes.refList && value.length < 1)) {
    ...
    if (valueComparableTypes.indexOf(propDesc.type) >= 0 && value != null) {
        let valueToCompare = propDesc.type === _blankConst.propertyTypes.date ? new Date(value).valueOf() : value;

        if (propDesc.min != null) {
            let validator = propDesc.min;
            let min = validator.getValue(user, item, baseItem);
            min = propDesc.type === _blankConst.propertyTypes.date ? new Date(min).valueOf() : min;
        ...
        if (propDesc.max != null) {
            ...
            max = propDesc.type === _blankConst.propertyTypes.date ? new Date(max).valueOf() : max;
...
class validation {
    ...
    static getPlainPropsNames(invalidProps, res, prefix) {
        ...
        for (let propName of Object.keys(invalidProps)) {
            for (let error of invalidProps[propName]) {
                if (error.type === _blankConst.validityErrors.INNER_ERROR) {
    ...
    static getValidator(propDesc, validatorName, currentI18n) {
        ...
        let validator = new Validator(_blankConst.baseValidators[validatorName].type, _blankConst.baseValidators[validatorName].message);
        ...
        if (propDesc.type === _blankConst.propertyTypes.date && model.$validatorValue) {
