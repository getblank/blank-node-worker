...
const valueComparableTypes = [
    _constants.propertyTypes.int,
    _constants.propertyTypes.float,
    _constants.propertyTypes.date,
];
const lengthComparableTypes = [
    _constants.propertyTypes.string,
    _constants.propertyTypes.password,
    _constants.propertyTypes.objectList,
];
...
const nonValidatedTypes = [
    _constants.propertyTypes.virtual,
    _constants.propertyTypes.virtualRefList,
    _constants.propertyTypes.comments,
    _constants.propertyTypes.action,
];
...
function validateProperty(storeName, propsDesc, item, propName, baseItem, user) {
    ...
    if (propDesc.required != null) {
        ...
        if (
            validator.getValue(user, item, baseItem) &&
            (value == null || value === "" || (propDesc.type === _constants.propertyTypes.refList && value.length < 1))
        ) {
...
class validation {
    ...
    static getValidator(propDesc, validatorName, currentI18n) {
        ...
        const validator = new Validator(
            _constants.baseValidators[validatorName].type,
            _constants.baseValidators[validatorName].message
        );
    ...
    static __getValidatorFunction(expression) {
        return new Function(
            ...
            "$item",
            "$baseItem",
            `$user = $user || {}; $item = $item || {}; return ${expression};`
        );
    }
}
