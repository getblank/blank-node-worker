...
class ConfigStore {
    ...
    async setup(config) {
        ...
        for (const storeName of Object.keys(this._config || {})) {
            ...
            if (storeDesc.type === "map") {
                continue;
            }

            const { dataSource } = storeDesc;
            if (dataSource) {
                this._dataSources.add(dataSource.type);
                switch (dataSource.type) {
                    ...
                    default:
                        throw new Error(
                            `dataSource type in store ${storeName} can by on of "mongo", "postgres" or "file"`
                        );
                }
            } else {
                const defaultDB = process.env.BLANK_DB;
                if (defaultDB) {
                    this._dataSources.add(defaultDB);
                    if (!["postgres", "mongo"].includes(defaultDB)) {
                        throw new Error(`Unknown default dataSource type "${defaultDB}" in env "BLANK_DB"`);
                    }

                    storeDesc.dataSource = { type: defaultDB };
                } else {
                    this._dataSources.add("mongo");
                    storeDesc.dataSource = { type: "mongo" };
                }
            }
        }
    ...
    async prepareProps(props, storeName, storeDesc, parentPropName) {
        for (const propName of Object.keys(props || {})) {
            ...
            if (propDesc.type === "virtual" && typeof propDesc.load !== "function") {
                ...
                propDesc.load = userScript.create(script, `${propName}_load`, ["$item", "$baseItem", "$user"], false);
            }
            ...
            if (typeof propDesc.default === "object" && propDesc.default.$expression != null) {
                propDesc.$default = new Function("$item", "$user", "$i18n", propDesc.default.$expression);
            }
            ...
            if (propDesc.type === "ref" || propDesc.type === "refList") {
                ...
                if (propDesc.showAddAction) {
                    ...
                    const actionDesc = {
                        ...
                        script: `
                            delete $data._id;
                            return $db.insert("${propDesc.store}", $data).then(res => res._id);
                        `,
                        clientPostScript:
                            propDesc.type === "ref"
                                ? `
                        ...
                        `
                                ...
                                ${propName}.push($result);
                                $setProperty("${propName}", ${propName});
                            `,
                        hideInHeader: true,
    ...
    getActionDesc(storeName, actionId) {
        ...
        const scriptArgs = ["$user", "$item", "$data", "$token", "$request", "$filter", "$context"];
