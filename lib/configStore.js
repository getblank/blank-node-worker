...
class ConfigStore {
    ...
    prepareAuthLifeCycle() {
        ...
        if (findUser && typeof findUser === "string") {
            auth.findUser = userScript.create(findUser, "_serverSettings_auth_findUser", ["$db", "login", "password", "hashedPassword"], true);
        }
        ...
        if (willSignIn && typeof willSignIn === "string") {
            auth.willSignIn = userScript.create(willSignIn, "_serverSettings_auth_willSignIn", ["$db", "$user", "$data"], true);
        }

        if (didSignIn && typeof didSignIn === "string") {
            auth.didSignIn = userScript.create(didSignIn, "_serverSettings_auth_didSignIn", ["$db", "$user", "$data"], true);
        }

        if (willSignOut && typeof willSignOut === "string") {
            auth.willSignOut = userScript.create(willSignOut, "_serverSettings_auth_willSignOut", ["$db", "$user", "$data"], true);
        }

        if (didSignOut && typeof didSignOut === "string") {
            auth.didSignOut = userScript.create(didSignOut, "_serverSettings_auth_didSignOut", ["$db", "$user", "$data"], true);
        }
    }
