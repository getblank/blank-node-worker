...
let {clientStoreDef} = require("./const");
let find = require("utils/find");
let configProcessor = require("configProcessor").default;
            ...
            if (propDesc.type === "virtualRefList") {
                switch (typeof propDesc.query) {
                    case "object":
                        ...
                        break;
                    case "string":
                        propDesc.query = new Function("$item", "$user", `$user = $user || {}; return ${propDesc.query};`);
                        break;
                }
            }
            if ((propDesc.type === "ref" || propDesc.type === "refList") && propDesc.populateIn) {
                if (typeof propDesc.populateIn === "string") {
                    ...
                    propDesc.populateIn.fn = new Function(arg, propDesc.populateIn.map);
                }
