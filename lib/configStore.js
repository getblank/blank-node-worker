...
class ConfigStore {
    ...
    prepareAuthLifeCycle() {
        ...
        if (willSignOut && typeof willSignOut === "string") {
            auth.willSignOut = userScript.create(willSignOut, "_serverSettings_auth_willSignOut", ["$db", "$user"], true);
        }

        if (didSignOut && typeof didSignOut === "string") {
            auth.didSignOut = userScript.create(didSignOut, "_serverSettings_auth_didSignOut", ["$db", "$user"], true);
        }
    }
