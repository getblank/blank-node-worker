...
class ConfigStore {
    ...
    setup(config) {
        ...
        for (const storeName of Object.keys(this._config || {})) {
            ...
            this.prepareProps(storeDesc.props, storeName, storeDesc);
    ...
    fillAppTitle() {
        if (!this._config["_commonSettings"]) {
            console.error("[fillAppTitle] _commonSettings does not exists in config");
            return;
        }

        const { entries } = this._config["_commonSettings"];
        if (!entries) {
            console.error("[fillAppTitle] etries does not exists in _commonSettings");
            return;
        }
    ...
    getDataSource(storeNameOrDesc, ee) {
        ...
        if (!dataSource.module) {
            try {
                dataSource.module = userScript.requireLib(dataSource.file)(storeName, ee);
            } catch (err) {
    ...
    prepareAuthLifeCycle() {
        ...
        if (checkPassword && typeof checkPassword === "string") {
            auth.checkPassword = userScript.create(checkPassword, "_serverSettings_auth_checkPassword", ["$user", "$data"]);
        }

        if (changePassword && typeof changePassword === "string") {
            auth.changePassword = userScript.create(changePassword, "_serverSettings_auth_changePassword", ["$user", "password", "newPassword"]);
        }

        if (willSignIn && typeof willSignIn === "string") {
            auth.willSignIn = userScript.create(willSignIn, "_serverSettings_auth_willSignIn", ["$db", "$user", "$data"]);
        }
        ...
        auth.createToken = userScript.create(createToken, "_serverSettings_auth_createToken", ["$db", "$user", "$prevUser", "$data"]);
        ...
        if (willSignOut && typeof willSignOut === "string") {
            auth.willSignOut = userScript.create(willSignOut, "_serverSettings_auth_willSignOut", ["$db", "$user", "$data"]);
        }

        if (didSignOut && typeof didSignOut === "string") {
            auth.didSignOut = userScript.create(didSignOut, "_serverSettings_auth_didSignOut", ["$db", "$user", "$data"]);
        }
    }

    prepareProps(props, storeName, storeDesc, parentPropName) {
        for (const propName of Object.keys(props || {})) {
            ...
            if ((propDesc.type === "ref" || propDesc.type === "refList")) {
                ...
                if (propDesc.showAddAction === true) {
                    ...
                    const actionDesc = {
                        _id: actionId,
                        label: `{{$i18n.$stores.${propDesc.store}.storeLabel}}: Новый документ`,
                        script: `
                            delete $data._id;
            ...
            if (propDesc.props) {
                this.prepareProps(propDesc.props, storeName, storeDesc, propName);
            }
        }
    }
    ...
    getActionDesc(storeName, actionId) {
        ...
        const res = JSON.parse(JSON.stringify(action));
        const scriptArgs = ["$user", "$item", "$data", "$token", "$request", "$filter"];
        res.script = userScript.create(res.script, `${storeName}_${res.storeAction ? "storeActions" : "actions"}_${actionId}`, scriptArgs);
    ...
    getItemEventHandler(storeName, event) {
        ...
        const handler = userScript.create(storeDesc.objectLifeCycle[event], `${storeName}_itemEvents_${event}`, ["$user", "$item", "$prevItem"]);
    ...
    getBaseConfig(lang) {
        const cs = JSON.parse(JSON.stringify(this._config["_commonSettings"]));
    ...
    __getUserProps(props, user, wsDesc) {
        ...
        for (const propName of Object.keys(props || {})) {
            const propDesc = props[propName];
            const access = auth.computeAccess(propDesc.access, user);
    ...
    __getUserActions(actions, user) {
        ...
        for (const actionDesc of (actions || [])) {
            if (auth.hasExecuteAccess(actionDesc.access, user)) {
                ...
                const groupAccess = auth.computeAccess(access, user);
                const _actionDesc = {
                    _id,
                    clientPostScript,
                    clientPreScript,
