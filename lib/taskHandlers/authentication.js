...
class Authentication extends TaskHandlerBase {
    run(storeName, user, args, cb) {
        if (args == null || (!args.social && !(args.login && (args.password || args.hashedPassword)))) {
            return cb(
                new UserError("Invalid args. Must be login:string and (password:string or hashedPassword:string)"),
                null
            );
        }
        switch (args.social) {
    ...
    _runHooks(auth, args, user, cb) {
        ...
        return auth.createToken(this.db, user, args).then(res => cb(null, res));
    }

    async _baseLogin(args, cb) {
        ...
        return getUser
            ...
            .then(res => {
                ...
                if (
                    ...
                    return cb(new UserError(i18n.get("_commonSettings.signIn.invalidUserData")), null);
                }
                ...
                hash.calc(key, user.password.salt, (err, key) => {
                    if (user.password.key != key) {
                        return cb(new UserError(i18n.get("_commonSettings.signIn.invalidPassword")), null);
                    }
