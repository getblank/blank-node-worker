...
class Authentication extends TaskHandlerBase {
    ...
    _baseLogin(args, cb) {
        ...
        const getUser = findUser ? findUser(this.db, login, password, hashedPassword) : this.db.get("users", { $or: [{ login }, { email: login }] }, { returnNull: true });
        return getUser
            .then(user => {
                if (!user || (!user.isActive && login !== "root")) {
                    return cb(new UserError(i18n.get("_commonSettings.signIn.userNotFound")), null);
                }
