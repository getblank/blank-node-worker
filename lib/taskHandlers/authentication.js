...
class Authentication extends TaskHandlerBase {
    ...
    _baseLogin({ login, password, hashedPassword }, cb) {
        this.db.get("users", { $or: [{ login: login }, { email: login }] }, { populate: true }, (err, res) => {
            if (err != null) {
                return cb(new UserError("User not found"), null);
            }

            if (!res.isActive && login !== "root") {
                return cb(new UserError("User not found"), null);
            }

            const { auth } = configStore.getStoreDesc("_serverSettings").entries;
            if (auth && auth.checkPassword) {
                try {
                    if (auth.checkPassword(res, password)) {
                        return cb(null, res);
                    }
                } catch (err) {
            ...
            if (typeof res.password !== "object" || res.password == null || !res.password.salt || !res.password.key) {
            ...
            hash.calc(key, res.password.salt, function (err, key) {
                if (res.password.key != key) {
