...
class PasswordReset extends TaskHandlerBase {
    run(storeName, _, args, cb) {
        ...
        let user;
        const password = args.hashedPassword ? args.hashedPassword : crypto.createHash("md5").update(args.password).digest("hex");
        this.db.get("users", { _passwordResetToken: args.token, isActive: true }, { noPopulate: true }).then(_user => {
            return this.db.set("users", { _id: _user._id, password, _passwordResetExpires: null, _passwordResetToken: null });
        }).catch(err => {
