...
class PasswordResetRequest extends TaskHandlerBase {
    run(storeName, _, args, cb) {
        ...
        let user;
        this.db.get("users", { "$or": [{ login: args.email }, { email: args.email }], "isActive": true }, { "noPopulate": true }).then(_user => {
            let now = new moment();
            ...
            return this.db.set("users", { _id: _user._id, _passwordResetExpires: now.toDate(), _passwordResetToken: uuid.v4() });
        }).catch(err => {
