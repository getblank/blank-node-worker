...
function del(storeName, _id, cb) {
    ...
    if (!fileStoreURL) {
        cb(new Error("file store is not registered"));
        return d;
    }

    const uri = url.parse(url.resolve(fileStoreURL, `${storeName}/${_id}/`));
...
function save(storeName, file, fileBuffer, cb) {
    ...
    const uri = url.parse(url.resolve(fileStoreURL, `${storeName}/${file._id}/`));
    const req = http.request({
        protocol: uri.protocol,
        hostname: uri.hostname,
        port: uri.port,
