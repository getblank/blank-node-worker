...
const libDir = process.env.BLANK_DATA_DIR ? path.join(process.env.BLANK_DATA_DIR, "lib") : path.join(process.cwd(), "var", "lib");
const waiting = [];
const coreModules = [
    "assert",
    "buffer",
            ...
            fs.emptyDir(libDir, (err) => {
                ...
                zip.forEach(function (relativePath, file) {
                    if (!file.dir && (path.extname(relativePath) === ".js") || path.extname(relativePath) === ".json") {
                        promises.push(file.async("string").then(res => {
                            if (path.basename(relativePath) === "package.json") {
                                try {
                                    ...
                                    if (p.main) {
                                        ...
                                        console.debug(`Extracted package.json: ${relativePath}. Module: ${moduleName}. Main path: ${mainPath}`);
                                ...
                                console.debug("Extracted: ", relativePath, " Code:", res.slice(0, 10).replace(/(\r?\n)/g), "...");
...
class UserScript {
    ...
    create(code, scriptName, args, sync) {
        ...
        try {
            ...
            console.debug("Can't create function in VM", err);
        }
