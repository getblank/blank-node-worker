...
const runTask = async task => {
    ...
    try {
        const res = await handler.run(task.store, user, task.args); // TODO: pass storeDesc into taskHandler instead of taskName
        taskCb(null, res);
    } catch (err) {
        taskCb(err, null);
    }
...
const sendTaskError = (task, err) => {
    ...
    if (process.env.BLANK_LOG_ERRORS === "true") {
        try {
            $db.logError(task.store, task.type, task.args, task.userId, err.stack ? err.stack : err);
        } catch (err) {
