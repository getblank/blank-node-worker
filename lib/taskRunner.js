...
function sendTaskError(task, err) {
    console.error(`${task.id || "?"} | ${task.store || "?"} | ${task.type || "?"} | ${Date.now() - task._started}ms | Task error:`, err);

    // SIC! to not stringify all requests when debug is turned off
    if (process.env.BLANK_DEBUG) {
        console.debug(`${task.id || "?"} | ${task.store || "?"} | ${task.type || "?"} | ${JSON.stringify(task.args)} | ${Date.now() - task._started}ms | Task error:`, err);
    }
...
function sendTaskResult(task, res) {
